 ...
 * @LastEditTime: 2025-04-30 16:25:28
 * @FilePath: \ahla-tkxt-web\src\components\dataManage\emergencyrescue\escapeline\index.vue
 * @Description: 避灾路线
 * @Description: 避灾路线添加新增、修改、删除、查询数据功能
 *  
-->
  ...
  <div class="escape">
    ...
    >
      <el-form ref="dialogGroupRef" :model="formGrop" :rules="rules" label-width="100px">
        <el-form-item label="所属矿区" prop="diggingsId">
          <el-select
          ...
          >
            <el-option
              v-for="item in diggingsIdOption"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
...
<script setup>
import { savehedgeline, edithedgeline, gethedgelinepage, dellhedgeline } from '@/api/escapelineApi'
import { mapserviceRegionPage } from '@/api'
...
// 所属矿区
const diggingsIdOption = ref([{ label: '周油坊矿区', value: 1 }])
// 点击的页大小
...
const submitFormGroup = () => {
  dialogGroupRef.value.validate((valid) => {
    ...
    if (titleFrom.value == '新增') {
      savehedgeline(formGrop).then((res) => {
        if (res.code == 200) {
      ...
      edithedgeline(formGrop).then((res) => {
        if (res.code == 200) {
...
// 获取列表
const getescapeList = () => {
  gethedgelinepage(queryParams).then((res) => {
