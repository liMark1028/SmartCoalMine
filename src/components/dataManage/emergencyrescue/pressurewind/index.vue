 ...
 * @LastEditTime: 2025-04-30 17:43:59
 * @FilePath: \ahla-tkxt-web\src\components\dataManage\emergencyrescue\pressurewind\index.vue
 * @Description: 压风自救
 * 
 *  
-->
...
<template>
  <div class="pressurewind">
    <div class="pressurewind_header">
    ...
    <div class="pressurewind_contnet">
...
import {
  mapserviceWindPage,
  mapserviceWind,
  mapserviceWindUpdate,
  mapserviceWindDetele,
...
const submitFormGroup = () => {
  dialogGroupRef.value.validate(async (valid) => {
    ...
    if (titleFrom.value == '新增') {
      ...
      await mapserviceWind(data)
      ElMessage.success('新增成功')
    } else {
      await mapserviceWindUpdate(formGrop)
...
const dell = (row) => {
  ElMessageBox.confirm('是否确认删除名称为"' + row.name + '"的数据项?')
    .then(() => {
      mapserviceWindDetele({ windRescueDeviceId: row.id })
      _mapserviceWindPage()
...
// 获取接口
const _mapserviceWindPage = async () => {
  const data = {
    diggingsId: kqDataid.value,
    ...queryParams,
  }
  const res = await mapserviceWindPage(data)
  if (res.code == 200) {
