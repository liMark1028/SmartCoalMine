<!--
 * @Author: wangshiwei
 * @Date: 2025-04-18 10:53:10
 * @LastEditors: li yu mo
 * @LastEditTime: 2025-04-30 15:20:21
 * @FilePath: \ahla-tkxt-web\src\components\dataManage\resourcemanage\perspective\index.vue
 * @Description: 视角管理
 * 
 *  
-->
...
<script setup>
import {
  mapserviceVisualPage,
  mapserviceVisual,
  mapserviceVisualUpdate,
  mapserviceVisualDetele,
} from '@/api'
...
const submitFormGroup = () => {
  dialogGroupRef.value.validate(async (valid) => {
    ...
    if (titleFrom.value == '新增') {
      ...
      await mapserviceVisual(data)
      ElMessage.success('新增成功')
    } else {
      await mapserviceVisualUpdate(formGrop)
...
const dell = (row) => {
  ElMessageBox.confirm('是否确认删除数据?')
    .then(() => {
      mapserviceVisualDetele({ visualAngleId: row.id })
      _mapserviceVisualPage()
...
// 获取接口
const _mapserviceVisualPage = async () => {
  const data = {
    diggingsId: kqDataid.value,
    ...queryParams
  }
  const res = await mapserviceVisualPage(data)
  if (res.code == 200) {
